...
class Player:
    ...
    def __init__(self, player_id, pitch):
        ...
        if player_id == Player.PLAYER_RED:
            ...
            self._borders = ((pitch.i_min, (pitch.i_max - pitch.i_min)/2), (pitch.j_min, pitch.j_max))
            self._center = (200, 300)
        else:
            self.playerColor = Player.PLAYER_BLUE
            self._borders = (((pitch.i_max - pitch.i_min)/2, pitch.i_max), (pitch.j_min, pitch.j_max))
        ...
        self._mallet = Mallet(30, self._center[0], self._center[1], 100, self, self._borders)
